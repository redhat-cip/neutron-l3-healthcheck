# == Class: quantum_l3_healthcheck
#
# Installs & configure quantum-l3-healthcheck
#
# === Parameters
#
# [*sample_parameter*]
...
class quantum_l3_healthcheck (
  ...
  $db_connection       = 'mysql://localhost/ovs_quantum?charset=utf8'
) {
  ...
  quantum_l3_healthcheck_config {
    ...
    'DEFAULT/rpc_backend':         value => "quantum.openstack.common.rpc.${rpc_backend}";
    'database/connection':         value => $db_connection;
  }

  package { 'quantum-l3-healthcheck':
    ensure => present
  }

  service { 'quantum-l3-healthcheck':
    ensure  => running,
    enable  => true,
    require => Package['quantum-l3-healthcheck']
  }
